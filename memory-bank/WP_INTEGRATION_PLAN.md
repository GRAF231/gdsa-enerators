...
---
**✅ ЗАДАЧА 7 ЗАВЕРШЕНА: Создать шаблоны `template-contacts.php` и `template-design-epc.php`**
...
**### СТАТУС ВЫПОЛНЕНИЯ**
- [x] 1. Создан `template-contacts.php` с полной версткой страницы
- [x] 2. Создан `template-design-epc.php` с полной версткой страницы

**### ВЫПОЛНЕННЫЕ ИЗМЕНЕНИЯ**

#### template-contacts.php (создан):
- [x] Заголовок шаблона: `Template Name: Контакты`
- [x] Подключены функции: `get_header()` и `get_footer()`
- [x] Хлебные крошки через функцию `dsa_breadcrumbs()`
- [x] Класс основного контейнера: `main-content`
- [x] Перенесена полная верстка из `contacts.html`
- [x] Расставлены комментарии `<!-- ACF: ... -->` для 3 секций:
...
- [x] Контент включает:
  - Заголовок страницы
  - Основные контакты (левая колонка)
  - Офис продаж с картой (правая колонка)
  - Сетка из 25 филиалов по всей России
  - Форма запроса (заглушка для будущей CF7 интеграции)

#### template-design-epc.php (создан):
- [x] Заголовок шаблона: `Template Name: Design & EPC`
- [x] Подключены функции: `get_header()` и `get_footer()`
- [x] Хлебные крошки через функцию `dsa_breadcrumbs()`
- [x] Класс основного контейнера: `main-content`
- [x] Перенесена полная верстка из `design-epc.html`
...
- [x] Контент включает:
  ...
  - Блок про генподряд с изображением
  - Карточки лицензий и разрешений
  - Производственная статистика
  - Сетка услуг инженерного центра (5 карточек)
  - Процесс приобретения (5 шагов)
  - Сетка дополнительных опций (11 элементов)
  - Список сервисных услуг (6 пунктов)
  - Форма обратной связи (заглушка)
  - Сетка лицензий компании (8 карточек)

**### ДЛЯ ЗАВЕРШЕНИЯ ИНТЕГРАЦИИ (выполнить в админ-панели):**
1. Создать страницу "Контакты" в WordPress
2. Выбрать шаблон "Контакты" в атрибутах страницы
3. Создать страницу "Design & EPC" в WordPress
4. Выбрать шаблон "Design & EPC" в атрибутах страницы
5. Интеграция форм Contact Form 7 будет в отдельной задаче

**Статистика:**
- template-contacts.php: ~400 строк
- template-design-epc.php: ~600 строк
- **Всего: ~1000 строк кода**

**Результат:** Полнофункциональные шаблоны страниц с полной версткой, готовые к использованию в WordPress. Все блоки помечены комментариями для будущей интеграции с ACF и Contact Form 7.
...
---
**✅ ЗАДАЧА 8 ЗАВЕРШЕНА: Создать шаблоны `template-production.php` и `template-gratitude.php`**
...
**### СТАТУС ВЫПОЛНЕНИЯ**
- [x] 1. Создан `template-production.php` и назначен соответствующей странице
- [x] 2. Создан `template-gratitude.php` и назначен соответствующей странице
...
## ✅ ЗАДАЧА 8 ЗАВЕРШЕНА: Шаблоны "Производство" и "Благодарности"
...
- [x] Перенесена полная верстка из `src/html/production.html`:
  ...
  - 7 блоков цехов с описаниями и галереями изображений:
    - Цех резки по металлу
    - Современные машины 2022 года
    - Сборочный цех
    - Покрасочная камера
    - Цех сборки электрощитовой продукции
    - Конструкторский отдел
    - Цех изготовления топливных и масляных баков
...
- [x] Расставлены комментарии `<!-- ACF: ... -->` для 6 основных секций:
  - ACF: Основной блок производства
  - ACF: CTA блок экскурсии
  - ACF: Список производимой продукции
  - ACF: Блок цехов и оборудования
  - ACF: Преимущества производства
  - ACF: Менеджмент и план-график работ
  - ACF: Информация о контейнерах
...
- [x] Перенесена полная верстка из `src/html/gratitude.html`:
  ...
  - CTA блок с кнопками "Связаться с нами" и "Каталог продукции"
  - Модальное окно lightbox для просмотра изображений
- [x] Все пути к изображениям заменены на динамические:
  - Использование `get_template_directory_uri() . '/assets/img/gratitude/'`
